...
# 4 nodes x 8 H200, PP=1, DP=32, CP=1, EP=1, seq=2048, bf16, Megatron-FSDP.

# Launch command for running the benchmark from the root of this repository:
# srun_mcore --jobid=<id> -N4 --ntasks-per-node=1 -o logs/pretraining/qwen3.5-122b-a10b/h200-4n8g/pp1-dp32-cp1-ep1-seq2048-bf16-%J.log benchmarks/pretraining/qwen3.5-122b-a10b/h200-4n8g/pp1-dp32-cp1-ep1-seq2048-bf16.sh
...
MODEL_CONFIG[max-position-embeddings]=2048
...
INFRA_CONFIG[pipeline-model-parallel-size]=1
INFRA_CONFIG[context-parallel-size]=1
INFRA_CONFIG[expert-model-parallel-size]=1
INFRA_CONFIG[use-megatron-fsdp]=true
INFRA_CONFIG[init-model-with-meta-device]=true
INFRA_CONFIG[ckpt-format]=fsdp_dtensor
INFRA_CONFIG[recompute-granularity]=full
INFRA_CONFIG[recompute-method]=uniform
INFRA_CONFIG[recompute-num-layers]=1
